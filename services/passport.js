...
passport.use(new GoogleStrategy({
    ...
    User.findOne({googleID: profile.id}).then((existingUser) => { // Look through User collection for an instance already with the ID
            if (existingUser) { 
                ...
                new User({
                    ...
                    .save() // Save the user to the mongoDB
                    .then(user => done(null,user)); // Wait for asyncronious DB call to finish, and return the created user
            }        
        });
})
); 